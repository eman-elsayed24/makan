...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-17 bg-background-ivory border-b border-border">
      <div className="max-w-360 mx-auto px-4 sm:px-8 lg:px-20 h-full">
        <div className="flex items-center justify-between h-full gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              href="/favorites"
              className="relative w-10 h-10 flex items-center justify-center rounded-full text-primary-charcoal hover:bg-surface-beige transition-colors"
              aria-label="المفضلة"
            >
              <Heart size={22} strokeWidth={1.5} />
              {hydrated && count > 0 && (
                <span className="absolute -top-1 -left-1 w-5 h-5 bg-primary-olive text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                  {count}
                </span>
              )}
            </Link>
