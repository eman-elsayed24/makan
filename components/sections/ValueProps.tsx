      ...
      <div className="max-w-360 mx-auto px-4 sm:px-8 lg:px-20 py-8 sm:py-10 lg:py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {valueCards.map((card, index) => (
            <div
              key={index}
              className={`bg-card border border-border rounded-2xl p-5 flex items-center gap-3.5 transition-all duration-500 ease-out motion-reduce:transition-none ${
                isVisible
            ...
            >
              <div className="bg-muted rounded-xl w-11 h-11 flex items-center justify-center shrink-0">
                <Image
                  src={card.icon}
                  alt=""
                  width={22}
              ...
              <div className="flex-1 text-right space-y-1">
                <h3 className="font-bold text-[15px] leading-normal text-foreground">
                  {card.title}
                </h3>
                <p className="font-normal text-[12px] leading-normal text-muted-foreground">
                  {card.description}
                </p>
              </div>
            </div>
